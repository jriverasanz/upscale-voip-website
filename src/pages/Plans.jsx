...
const Plans = () => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer:
        "All plans include business hours email and chat support. Phone support is available for Business and Enterprise plans.",
    },
  ];
  ...
  return (
    <div className="font-body text-brand-charcoal min-h-screen bg-gray-50">
      ...
      <div className="bg-white">
        <div className="max-w-7xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8 text-center">
          ...
          <p className="mt-6 max-w-2xl mx-auto text-xl text-brand-charcoal/70">
            All the features you need to communicate with your team and
            customers. No hidden fees, no contracts.
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3 lg:gap-6">
          {plans.map((plan) => (
            <div
              ...
              onClick={() =>
                setClickedPlan(plan.name === clickedPlan ? null : plan.name)
              }
            >
              {plan.popular && (
                <div className="absolute top-0 right-0 bg-brand-violet text-white text-xs font-semibold px-3 py-1 rounded-bl-lg">
                  Most Popular
                </div>
              )}
              <div className="p-8 flex flex-col h-full">
                <h2
                  className={`text-2xl font-display ${plan.popular ? "text-brand-navy" : "text-brand-navy"}`}
                >
                  {plan.name}
                </h2>
                ...
                <div className="mt-6 flex items-baseline">
                  <span className="text-5xl font-display tracking-tight text-brand-navy">
                    {plan.price}
                  </span>
                  <span className="ml-1 text-lg font-medium text-brand-slate">
                    {plan.period}
                  </span>
                </div>
                <ul className="mt-8 space-y-3 flex-1">
                  {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-start">
                      <svg
                        className="h-5 w-5 text-brand-violet flex-shrink-0 mt-0.5"
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path
                      ...
                      <span className="ml-3 text-brand-charcoal">
                        {feature}
                      </span>
                    </li>
                  ))}
                ...
                <div className="mt-8">
                  <Link
                    to="/contact"
                    className={`w-full block text-center px-6 py-3 border border-transparent text-base font-medium text-white ${
                      plan.popular
                        ? "bg-[#97499c] hover:bg-[#7a387d]"
                        : "bg-[#97499c] hover:bg-[#7a387d]"
            ...
            {includedFeatures.map((feature, index) => (
              <div key={index} className="bg-white p-6 rounded-lg shadow-sm">
                <div className="flex items-center">
                  <svg
                    className="h-5 w-5 text-brand-violet flex-shrink-0"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
      ...
      {/* CTA Section */}
      <div className="bg-gradient-to-r from-brand-navy to-brand-violet">
        <div className="max-w-2xl mx-auto text-center py-16 px-4 sm:py-20 sm:px-6 lg:px-8">
          <h2 className="font-display text-3xl lg:text-4xl text-white">
            <span className="block">
              Start your new communication journey now.
            </span>
          </h2>
          ...
          <Link
            to="/contact"
            className="mt-8 w-full flex justify-center items-center px-8 py-3 border border-transparent text-base font-medium text-brand-violet bg-white hover:bg-brand-softblue md:py-4 md:text-lg md:px-10 rounded-[80px]"
          >
            Get Started
          </Link>
        </div>
